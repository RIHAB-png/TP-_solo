...
int Menu_Load()
{
	...
	if(!s_title || !s_fbmp){
	  char * f_title = buildFilenameFromHome("/.snake/pics/title.png");
	  s_title = IMG_Load(f_title);
	  char * f_fbmp = buildFilenameFromHome("/.snake/pics/font.bmp");
	  s_fbmp = IMG_Load(f_fbmp);
	  free(f_title);
	  free(f_fbmp);
	  if(!s_title || !s_fbmp){
	    Menu_Free();
